        ...
        <x-card>
            <div class="card-body">
                ...
                <p class="fs-6 mb-0">
                    ...
                    <span class="float-end">{{ $user->contact->facebook ?? 'Chưa cập nhật' }}</span>
                </p>
                <p class="fs-6 mb-0">
                    <i class="fa-solid fa-envelope"></i> Zalo: 
                    <span class="float-end">{{ $user->contact->zalo ?? 'Chưa cập nhật' }}</span>
                </p>
            </div>
        </x-card>
        <x-card>
            ...
            <div class="card-body">
                ...
                @else
                    <span>Shop chưa cập nhật thông tin ngân hàng</span>
                @endif
            </div>
        </x-card>
    </div>
...
@if($users != null)
    @foreach($users->where('id', $user->id) as $value)
        @if(count($value->reviews->where('author_id', auth()->user()->id)->where('user_id', $value->id)) 
            ...
            <x-card>
                <div class="card-body">
                    <x-form type="post" action="{{ route('reviews.create') }}" :validate="true">
...
<x-card>
    <x-slot name="header">
        <span class="text-white">
            ...
            <span class="">Đánh giá của các shop</span>
            <p class="float-end m-0 rating-box">
                <span class="text-warning"> 
                    @if ($user->reviews->where('status', 1)->count() > 0)
                        {{ round($user->reviews->where('status', 1)->sum('rating') / $user->reviews->where('status', 1)->count(),1) }}
                    @else 0 @endif
    ...
    <div class="card-body">
        @foreach($user->reviews->where('status', 1) as $value)
